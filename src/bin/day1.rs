fn report_repair<T>(mut iter: T, depth: i32, sum: i32) -> Option<i32>
where
    T: std::iter::Iterator<Item = i32> + Clone,
{
    if depth == 1 {
        return iter.clone().find(|val| *val == sum);
    }

    while let Some(val) = iter.next() {
        if let Some(target) = report_repair(iter.clone(), depth - 1, sum - val) {
            return Some(val * target);
        }
    }
...
fn main() {
    let input = vec![
        1313, 1968, 1334, 1566, 820, 1435, 1369, 1230, 1383, 1816, 1396, 1974, 1911, 1989, 1824,
    ...
    println!("{:?}", report_repair(input.into_iter(), 3, 2020));
}
